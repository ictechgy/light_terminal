...
| 모바일에서 agent 출력 확인 | `LTERM_MOBILE=1 lterm resume codex-lterm` | 강제 사용은 `--mobile`, raw 우회는 `--raw` |
...
`lterm resume` / `lterm open`의 기본 attach 정책은 `LTERM_ATTACH_MODE=auto`입니다. 데스크톱에서는 기존처럼 raw PTY attach를 사용합니다. 반면 Termius 계열 클라이언트이거나 `LTERM_MOBILE=1`이 설정된 클라이언트가 agent 세션으로 보이는 대상에 붙으면, 일반 화면(normal screen)의 모바일 transcript view로 자동 전환됩니다. 이 transcript는 정제된 capture 출력과 기존 `input` 경로를 사용합니다. alternate screen에 들어가지 않고, attached client로 계산되지 않으며, PTY 크기도 바꾸지 않습니다. CLI flag가 환경 변수보다 우선합니다. 기존 raw 경로를 강제로 쓰려면 `--raw` 또는 `LTERM_ATTACH_MODE=raw`, transcript를 강제로 쓰려면 `--mobile` 또는 `LTERM_ATTACH_MODE=mobile`을 사용하세요. transcript의 표시 범위와 갱신 주기는 `--tail`, `--refresh`, `--read-only`로 조정할 수 있습니다.
...
위의 일반 alias는 tmux 용어를 몰라도 agent terminal을 일상적으로 다루기 쉽게 하기 위한 표면입니다. `sessions`는 영속 작업을 나열하고, `processes`는 child process tree를 확인하고, `logs`는 정제된 scrollback을 읽고, `compose`는 정제된 scrollback과 하단 고정 prompt로 텍스트를 commit할 수 있게 하며, 모바일 transcript attach는 긴 agent 출력을 휴대폰의 기본 scrollback으로 읽을 수 있게 해 줍니다. `wait` / `watch`는 marker 또는 종료 조건을 script와 agent가 관측할 수 있게 하고, `input`은 대상 PTY에 텍스트를 씁니다. `mobile`은 `compose`의 visible alias입니다. 호환 이름 `list` / `ls`, `ps`, `capture`, `send`는 스크립트와 기존 사용 습관에서도 계속 사용할 수 있습니다.
