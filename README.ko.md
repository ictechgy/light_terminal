...
바꿀 수 있으며, text mode에서 match가 없으면 성공 종료하면서 출력하지 않습니다.
모바일 transcript mode에서는 `/grep QUERY`를 입력하면 matching line을 로컬로
보여 주며, 해당 입력은 PTY로 전달되지 않습니다. match가 없으면
