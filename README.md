...
```bash
cargo install --locked --git https://github.com/ictechgy/light_terminal --tag v1.0.17
```
...
| Install AI CLI statusline badges | `lterm install-ai-statusline` | Installs supported command-statusline integrations for `LTERM_SESSION` / `LTERM_PANE`; currently Claude/OMC is written idempotently, while Codex is reported as skipped because its TUI status line exposes fixed built-in items |
