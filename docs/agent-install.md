...
Current release tag to prefer for source installs: v1.0.32
...
- Prefer the least surprising package manager for this machine:
  ...
  - fallback with Rust/Cargo: `cargo install --git https://github.com/ictechgy/light_terminal --tag v1.0.32`
...
- One recommended first command for my agent workflow, for example:
  ...
  - `lterm agent grok --name grok-main` (if a `grok` binary is on PATH)
