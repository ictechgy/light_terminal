...
- Treat major-version updates, new crates, build-script changes, and crates that
  ...
  design-review items.
- PR CI runs a version-checked `cargo audit` against the locked dependency graph
  and caches the audit binary by runner image/architecture to keep the gate
  reproducible without adding a large source-build cost to every run. Before
  tagging a release, also run
